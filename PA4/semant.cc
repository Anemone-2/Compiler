...
void method_class::CheckFeatureType() {
  ...
  if (classtable->m_classes.find(return_type) == classtable->m_classes.end() &&
    ...
    classtable->semant_error(curr_class) << "Error! return type " << return_type
...
Symbol static_dispatch_class::CheckExprType() {
  ...
  if (method != NULL) {
    ...
    Expressions actuals = actual;

    // 1. 检查参数数量是否匹配 (解决参数数量不匹配的错误报告)
    ...
    // 2. 类型检查循环：使用形参列表长度来避免越界
    int max_i = formals->len();

    for (int i = actuals->first(); actuals->more(i); i = actuals->next(i)) {
      Symbol actual_type = actuals->nth(i)->CheckExprType();

      // 只有当索引 i 在 formal 列表的有效范围内时，才进行类型检查
      if (i < max_i) {
        Symbol formal_type = formals->nth(i)->GetType();
        if (classtable->CheckInheritance(formal_type, actual_type) == false) {
          error = true;
          classtable->semant_error(curr_class->get_filename(), actuals->nth(i))
              << "In call of method " << name << ", type " << actual_type
              << " of parameter " << formals->nth(i)->GetName()
...
Symbol dispatch_class::CheckExprType() {
  ...
  if (method != NULL) {
    ...
    for (int i = actuals->first(); actuals->more(i); i = actuals->next(i)) {
      ...
      if (i < max_i) { // 检查 i 是否在 formal 列表的有效范围内
        Symbol formal_type = formals->nth(i)->GetType();
        if (classtable->CheckInheritance(formal_type, actual_type) == false) {
          error = true;
          classtable->semant_error(curr_class->get_filename(), actuals->nth(i))
...
Symbol new__class::CheckExprType() {
  if (type_name != SELF_TYPE &&
      classtable->m_classes.find(type_name) == classtable->m_classes.end()) {
    classtable->semant_error(curr_class)
